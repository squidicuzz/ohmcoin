...
CzOHMCWallet::CzOHMCWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zohmc seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZOHMCSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZOHMCSeed_deprecated()) {
                    LogPrintf("%s: Updated zOHMC seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zohmc seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zohmc wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzOHMCWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZOHMCCount(nCountLastUsed);
    else if (!walletdb.ReadZOHMCCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzOHMCWallet::Lock()
{
    seedMaster = 0;
}

void CzOHMCWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzOHMCWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZOHMC(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzOHMCWallet::LoadMintPoolFromDB()
{
...
void CzOHMCWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzOHMCWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzOHMCWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zohmcTracker->HasPubcoinHash(pMint.first)) {
...
bool CzOHMCWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZOHMC(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
...
void CzOHMCWallet::SeedToZOHMC(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzOHMCWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzOHMCWallet::GenerateDeterministicZOHMC(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzOHMCWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzOHMCWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
