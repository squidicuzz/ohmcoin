...
// Copyright (c) 2017 The Ohmcoin developers
...

    {
        ...
        for (map<uint256, CTxMemPoolEntry>::iterator mi = mempool.mapTx.begin();
            ...
            for (const CTxIn& txin : tx.vin) {
                ...
                if (tx.IsZerocoinSpend()) {
                    ...
                    // zOHMC spends can have very large priority, use non-overflowing safe functions
        ...
        while (!vecPriority.empty()) {
            ...
            if (tx.IsZerocoinSpend()) {
                ...
                //This zOHMC serial has already been included in the block, do not add this tx.
                if (fDoubleSerial)
                    continue;
            }
        ...
        if (!fProofOfStake) {
            //Karmanode and general budget payments
        ...
        if (fZerocoinActive) {
            ...
            if (nHeight >= pCheckpointCache.first || pCheckpointCache.second.first != hashBlockLastAccumulated) {
                //For the period before v2 activation, zOHMC will be disabled and previous block's checkpoint is all that will be needed
...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("OhmcoinMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("OhmcoinMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("OhmcoinMiner started\n");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        if (fProofOfStake) {
            ...
            while (chainActive.Tip()->nTime < 1504595227 || vNodes.empty() || pwallet->IsLocked() ||
                   nReserveBalance >= pwallet->GetBalance() || !karmanodeSync.IsSynced()) {
        ...
        LogPrintf("Running OhmcoinMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
