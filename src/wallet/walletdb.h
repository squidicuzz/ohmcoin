...
public:
    ...
    bool WriteMSettings(bool fMultiSendStake, bool fMultiSendKarmanode, int nLastMultiSendHeight);
    ...
    bool WriteZOHMCSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZOHMCSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZOHMCSeed_deprecated(uint256& seed);
    bool EraseZOHMCSeed();
    bool EraseZOHMCSeed_deprecated();

    bool WriteZOHMCCount(const uint32_t& nCount);
    bool ReadZOHMCCount(uint32_t& nCount);
