...
UniValue getnewaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() > 2)
        throw runtime_error(
                ...
                "\nResult:\n"
                "\"ohmcoinaddress\"    (string) The new ohmcoin address\n"
                "\nExamples:\n" +
...
UniValue getaccountaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
                ...
                "\nResult:\n"
                "\"ohmcoinaddress\"   (string) The account ohmcoin address\n"
                "\nExamples:\n" +
...
UniValue getrawchangeaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw runtime_error(
                ...
                "\nReturns a new Ohmcoin address, for receiving change.\n"
...
UniValue setaccount(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
                ...
                "\nArguments:\n"
                "1. \"ohmcoinaddress\"  (string, required) The ohmcoin address to be associated with an account.\n"
...
UniValue getaccount(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
                ...
                "\nArguments:\n"
                "1. \"ohmcoinaddress\"  (string, required) The ohmcoin address for account lookup.\n"
                "\nResult:\n"
...
UniValue getaddressesbyaccount(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
                ...
                "  \"ohmcoinaddress\"  (string) a ohmcoin address associated with the given account\n"
                "  ,...\n"
                "]\n"
                "\nExamples:\n" +
...
UniValue sendtoaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 4)
        throw runtime_error(
                "sendtoaddress \"ohmcoinaddress\" amount ( \"comment\" \"comment-to\" )\n"
                "\nSend an amount to a given address. The amount is a real and is rounded to the nearest 0.00000001\n" +
                HelpRequiringPassphrase() +
                "\nArguments:\n"
                "1. \"ohmcoinaddress\"  (string, required) The ohmcoin address to send to.\n"
                "2. \"amount\"      (numeric, required) The amount in ohmcoin to send. e.g. 0.1\n"
...
UniValue sendtoaddressix(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 4)
        throw runtime_error(
                "sendtoaddressix \"ohmcoinaddress\" amount ( \"comment\" \"comment-to\" )\n"
                "\nSend an amount to a given address. The amount is a real and is rounded to the nearest 0.00000001\n" +
                HelpRequiringPassphrase() +
                "\nArguments:\n"
                "1. \"ohmcoinaddress\"  (string, required) The ohmcoin address to send to.\n"
...
UniValue listaddressgroupings(const UniValue& params, bool fHelp)
{
    if (fHelp)
        throw runtime_error(
                ...
                "  [\n"
                "    [\n"
                "      \"ohmcoinaddress\",     (string) The ohmcoin address\n"
                "      amount,                 (numeric) The amount in ohmcoin\n"
...
UniValue signmessage(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 2)
        throw runtime_error(
                ...
                HelpRequiringPassphrase() + "\n"
                                             "\nArguments:\n"
                                             "1. \"ohmcoinaddress\"  (string, required) The ohmcoin address to use for the private key.\n"
...
UniValue getreceivedbyaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
                "getreceivedbyaddress \"ohmcoinaddress\" ( minconf )\n"
                "\nReturns the total amount received by the given ohmcoinaddress in transactions with at least minconf confirmations.\n"
                "\nArguments:\n"
                "1. \"ohmcoinaddress\"  (string, required) The ohmcoin address for transactions.\n"
                "2. minconf             (numeric, optional, default=1) Only include transactions confirmed at least this many times.\n"
                "\nResult:\n"
                "amount   (numeric) The total amount in ohmcoin received at this address.\n"
                "\nExamples:\n"
...
UniValue getreceivedbyaccount(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
                ...
                "2. minconf          (numeric, optional, default=1) Only include transactions confirmed at least this many times.\n"
                "\nResult:\n"
                "amount              (numeric) The total amount in ohmcoin received for this account.\n"
                "\nExamples:\n"
...
UniValue movecmd(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 3 || params.size() > 5)
        throw runtime_error(
                ...
                "\nExamples:\n"
                "\nMove 0.01 ohmcoin from the default account to the account named tabby\n" +
                HelpExampleCli("move", "\"\" \"tabby\" 0.01") +
                "\nMove 0.01 ohmcoin timotei to akiko with a comment and funds have 6 confirmations\n" + HelpExampleCli("move", "\"timotei\" \"akiko\" 0.01 6 \"happy birthday!\"") +
...
UniValue sendfrom(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 3 || params.size() > 6)
        throw runtime_error(
                "sendfrom \"fromaccount\" \"toohmcoinaddress\" amount ( minconf \"comment\" \"comment-to\" )\n"
                "\nSent an amount from an account to a ohmcoin address.\n"
                "The amount is a real and is rounded to the nearest 0.00000001." +
                HelpRequiringPassphrase() + "\n"
                                             "\nArguments:\n"
                                             "1. \"fromaccount\"       (string, required) The name of the account to send funds from. May be the default account using \"\".\n"
                                             "2. \"toohmcoinaddress\"  (string, required) The ohmcoin address to send funds to.\n"
                                             "3. amount                (numeric, required) The amount in ohmcoin. (transaction fee is added on top).\n"
                                             ...
                                             "\"transactionid\"        (string) The transaction id.\n"
                                             "\nExamples:\n"
                                             "\nSend 0.01 ohmcoin from the default account to the address, must have at least 1 confirmation\n" +
    ...
    if (fHelp || params.size() < 2 || params.size() > 4)
        throw runtime_error(
                ...
                HelpRequiringPassphrase() + "\n"
                                             ...
                                             "    {\n"
                                             "      \"address\":amount   (numeric) The ohmcoin address is the key, the numeric amount in ohmcoin is the value\n"
                                             "      ,...\n"
                                             "    }\n"
...
UniValue addmultisigaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 3) {
        string msg = "addmultisigaddress nrequired [\"key\",...] ( \"account\" )\n"
                     ...
                     "2. \"keysobject\"   (string, required) A json array of ohmcoin addresses or hex-encoded public keys\n"
                     "     [\n"
                     "       \"address\"  (string) ohmcoin address or hex-encoded public key\n"
                     "       ...,\n"
                     "     ]\n"
                     "3. \"account\"      (string, optional) An account to assign the addresses to.\n"

                     "\nResult:\n"
                     "\"ohmcoinaddress\"  (string) A ohmcoin address associated with the keys.\n"
...
UniValue addwitnessaddress(const UniValue& params, bool fHelp)
{
    ...
    if (!IsSporkActive(SPORK_20_SEGWIT_ACTIVATION) && !GetBoolArg("-walletprematurewitness", false)) {
...
UniValue listreceivedbyaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() > 3)
        throw runtime_error(
                ...
                "    \"amount\" : x.xxx,                  (numeric) The total amount in ohmcoin received by the address\n"
...
UniValue listtransactions(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() > 4)
        throw runtime_error(
                ...
                "    \"address\":\"ohmcoinaddress\",    (string) The ohmcoin address of the transaction. Not present for \n"
                "                                                move transactions (category = move).\n"
                "    \"category\":\"send|receive|move\", (string) The transaction category. 'move' is a local (off blockchain)\n"
                "                                                transaction between accounts, and not associated with an address,\n"
                "                                                transaction id or block. 'send' and 'receive' transactions are \n"
                "                                                associated with an address, transaction id and block details\n"
                "    \"amount\": x.xxx,          (numeric) The amount in ohmcoin. This is negative for the 'send' category, and for the\n"
                "                                         'move' category for moves outbound. It is positive for the 'receive' category,\n"
                "                                         and for the 'move' category for inbound funds.\n"
                "    \"vout\" : n,               (numeric) the vout value\n"
                "    \"fee\": x.xxx,             (numeric) The amount of the fee in ohmcoin. This is negative and only available for the \n"
...
UniValue listsinceblock(const UniValue& params, bool fHelp)
{
    if (fHelp)
        throw runtime_error(
                ...
                "    \"address\":\"ohmcoinaddress\",    (string) The ohmcoin address of the transaction. Not present for move transactions (category = move).\n"
                "    \"category\":\"send|receive\",     (string) The transaction category. 'send' has negative amounts, 'receive' has positive amounts.\n"
                "    \"amount\": x.xxx,          (numeric) The amount in ohmcoin. This is negative for the 'send' category, and for the 'move' category for moves \n"
                "                                          outbound. It is positive for the 'receive' category, and for the 'move' category for inbound funds.\n"
                "    \"vout\" : n,               (numeric) the vout value\n"
                "    \"fee\": x.xxx,             (numeric) The amount of the fee in ohmcoin. This is negative and only available for the 'send' category of transactions.\n"
...
UniValue gettransaction(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
                ...
                "\nResult:\n"
                "{\n"
                "  \"amount\" : x.xxx,        (numeric) The transaction amount in ohmcoin\n"
                ...
                "      \"address\" : \"ohmcoinaddress\",   (string) The ohmcoin address involved in the transaction\n"
                "      \"category\" : \"send|receive\",    (string) The category, either 'send' or 'receive'\n"
                "      \"amount\" : x.xxx                  (numeric) The amount in ohmcoin\n"
                "      \"vout\" : n,                       (numeric) the vout value\n"
                "    }\n"
                "    ,...\n"
                "  ],\n"
...
UniValue walletpassphrase(const UniValue& params, bool fHelp)
{
    if (pwalletMain->IsCrypted() && (fHelp || params.size() < 2 || params.size() > 3))
        throw runtime_error(
                ...
                "This is needed prior to performing transactions related to private keys such as sending OHMCs\n"
                "\nArguments:\n"
...
UniValue encryptwallet(const UniValue& params, bool fHelp)
{
    if (!pwalletMain->IsCrypted() && (fHelp || params.size() != 1))
        throw runtime_error(
                ...
                "\nNow set the passphrase to use the wallet, such as for signing or sending OHMCs\n" + HelpExampleCli("walletpassphrase", "\"my pass phrase\"") +
                "\nNow we can so something like sign\n" + HelpExampleCli("signmessage", "\"ohmcoinaddress\" \"test message\"") +
    ...
    StartShutdown();
    return "Wallet encrypted; Ohmcoin server stopping, restart to run with encrypted wallet. The keypool has been flushed and a new HD seed was generated (if you are using HD). You need to make a new backup.";
}
...
UniValue lockunspent(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
                ...
                "A locked transaction output will not be chosen by automatic coin selection, when spending OHMCs.\n"
...
UniValue settxfee(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 1)
        throw runtime_error(
                ...
                "\nArguments:\n"
                "1. amount         (numeric, required) The transaction fee in OHMC/kB rounded to the nearest 0.00000001\n"
                "\nResult\n"
...
UniValue autocombinerewards(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 1 || (fEnable && params.size() != 2) || params.size() > 2)
        throw runtime_error(
                ...
                "\nWallet will automatically monitor for any coins with value below the threshold amount, and combine them if they reside with the same Ohmcoin address\n"
...
UniValue printMultiSend()
{
    ...
    act.push_back(Pair("MultiSendKarmanode Activated?", pwalletMain->fMultiSendKarmanodeReward));
    ret.push_back(act);
...
UniValue multisend(const UniValue& params, bool fHelp)
{
    ...
    if (params.size() == 1) {
        ...
        if (strCommand == "print") {
            ...
            if (IsValidDestinationString(pwalletMain->vMultiSend[0].first)) {
                ...
                if (!walletdb.WriteMSettings(true, pwalletMain->fMultiSendKarmanodeReward, pwalletMain->nLastMultiSendHeight)) {
                    UniValue obj(UniValue::VOBJ);
        ...
        } else if (strCommand == "enablekarmanode" || strCommand == "activatekarmanode") {
            if (pwalletMain->vMultiSend.size() < 1)
                throw JSONRPCError(RPC_INVALID_REQUEST, "Unable to activate MultiSend, check MultiSend vector");

            if (IsValidDestinationString(pwalletMain->vMultiSend[0].first)) {
                pwalletMain->fMultiSendKarmanodeReward = true;
    ...
    if (fHelp || params.size() != 2)
        throw runtime_error(
                ...
                "multisend <Ohmcoin Address> <percent>\n"
                ...
                " enablekarmanode/activatekarmanode - activates the current MultiSend vector to be activated on karmanode rewards\n"
...
UniValue getzerocoinbalance(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() != 0)
        throw runtime_error(
                "getzerocoinbalance\n"
                "\nReturn the wallet's total zOHMC balance.\n" +
                HelpRequiringPassphrase() + "\n"

                                             "\nResult:\n"
                                             "amount         (numeric) Total zOHMC balance.\n"
...
UniValue listmintedzerocoins(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() != 0)
        throw runtime_error(
                "listmintedzerocoins\n"
                "\nList all zOHMC mints in the wallet.\n" +
                HelpRequiringPassphrase() + "\n"
    ...
    CWalletDB walletdb(pwalletMain->strWalletFile);
    set<CMintMeta> setMints = pwalletMain->zohmcTracker->ListMints(true, true, true);
...
UniValue listspentzerocoins(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() != 0)
        throw runtime_error(
                "listspentzerocoins\n"
                "\nList all the spent zOHMC mints in the wallet.\n" +
                HelpRequiringPassphrase() + "\n"
...
UniValue mintzerocoin(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
                ...
                "\nMint the specified zOHMC amount\n" +
                HelpRequiringPassphrase() + "\n"

                                             "\nArguments:\n"
                                             "1. amount      (numeric, required) Enter an amount of OHMC to convert to zOHMC\n"
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_21_ZEROCOIN_MAINTENANCE_MODE))
        throw JSONRPCError(RPC_WALLET_ERROR, "zOHMC is currently disabled due to maintenance.");
...
UniValue spendzerocoin(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() > 5 || params.size() < 4)
        throw runtime_error(
                ...
                HelpRequiringPassphrase() + "\n"
                                             ...
                                             "    {\n"
                                             "      \"value\": amount,         (numeric) Value in OHMC.\n"
                                             "      \"address\": \"xxx\"         (string) OHMC address or \"zerocoinmint\" for reminted change.\n"
                                             "    }\n"
                                             "    ,...\n"
                                             "  ]\n"
                                             "}\n"
    ...
    bool fMintChange = params[1].get_bool();        // Mint change to zOHMC
...
UniValue resetmintzerocoin(const UniValue& params, bool fHelp)
{
    ...
    CzOHMCTracker* zohmcTracker = pwalletMain->zohmcTracker.get();
    set<CMintMeta> setMints = zohmcTracker->ListMints(false, false, true);
...
UniValue resetspentzerocoin(const UniValue& params, bool fHelp)
{
    ...
    CWalletDB walletdb(pwalletMain->strWalletFile);
    CzOHMCTracker* zohmcTracker = pwalletMain->zohmcTracker.get();
    set<CMintMeta> setMints = zohmcTracker->ListMints(false, false, false);
    ...
    for (CZerocoinSpend spend : listUnconfirmedSpends) {
        for (auto& meta : setMints) {
            if (meta.hashSerial == GetSerialHash(spend.GetSerial())) {
                zohmcTracker->SetPubcoinNotUsed(meta.hashPubcoin);
...
UniValue exportzerocoins(const UniValue& params, bool fHelp)
{
    if(fHelp || params.empty() || params.size() > 2)
        throw runtime_error(
                ...
                HelpRequiringPassphrase() + "\n"
                                             ...
                                             "2. \"denomination\"         (integer, optional) Export a specific denomination of zOHMC\n"
                                             ...
                                             "    \"v\": version,   (numeric) The version of the zOHMC\n"
                                             "    \"k\": \"privkey\"  (string) The zOHMC private key (V2+ zOHMC only)\n"
                                             "  }\n"
                                             "  ,...\n"
                                             "]\n"
    ...
    CzOHMCTracker* zohmcTracker = pwalletMain->zohmcTracker.get();
    set<CMintMeta> setMints = zohmcTracker->ListMints(!fIncludeSpent, false, false);
...
UniValue importzerocoins(const UniValue& params, bool fHelp)
{
    if(fHelp || params.size() == 0)
        throw runtime_error(
                ...
                HelpRequiringPassphrase() + "\n"
                                             ...
                                             "  \"value\": amount    (numeric) The total zOHMC value of zerocoin mints that were added\n"
                                             "}\n"
    ...
    for (unsigned int idx = 0; idx < arrMints.size(); idx++) {
        ...
        pwalletMain->zohmcTracker->Add(mint, true);
        count++;
...
UniValue reconsiderzerocoins(const UniValue& params, bool fHelp)
{
    if(fHelp || !params.empty())
        throw runtime_error(
                "reconsiderzerocoins\n"
                "\nCheck archived zOHMC list to see if any mints were added to the blockchain.\n" +
                HelpRequiringPassphrase() + "\n"
...
UniValue setzohmcseed(const UniValue& params, bool fHelp)
{
    if(fHelp || params.size() != 1)
        throw runtime_error(
                "setzohmcseed \"seed\"\n"
                "\nSet the wallet's deterministic zohmc seed to a specific value.\n" +
                HelpRequiringPassphrase() + "\n"

                                             "\nArguments:\n"
                                             "1. \"seed\"        (string, required) The deterministic zohmc seed.\n"

                                             "\nResult\n"
                                             "\"success\" : b,  (boolean) Whether the seed was successfully set.\n"

                                             "\nExamples\n" +
                HelpExampleCli("setzohmcseed", "63f793e7895dd30d99187b35fbfb314a5f91af0add9e0a4e5877036d1e392dd5") +
                HelpExampleRpc("setzohmcseed", "63f793e7895dd30d99187b35fbfb314a5f91af0add9e0a4e5877036d1e392dd5"));
    ...
    CzOHMCWallet* zwallet = pwalletMain->getZWallet();
...
UniValue getzohmcseed(const UniValue& params, bool fHelp)
{
    if(fHelp || !params.empty())
        throw runtime_error(
                "getzohmcseed\n"
                ...
                HelpRequiringPassphrase() + "\n"
                                             ...
                                             "\nResult\n"
                                             "\"seed\" : s,  (string) The deterministic zOHMC seed.\n"

                                             "\nExamples\n" +
                HelpExampleCli("getzohmcseed", "") + HelpExampleRpc("getzohmcseed", ""));
...
UniValue generatemintlist(const UniValue& params, bool fHelp)
{
    if(fHelp || params.size() != 2)
        throw runtime_error(
                "generatemintlist\n"
                "\nShow mints that are derived from the deterministic zOHMC seed.\n" +
                HelpRequiringPassphrase() + "\n"

                                             "\nArguments\n"
                                             "1. \"count\"  : n,  (numeric) Which sequential zOHMC to start with.\n"
                                             "2. \"range\"  : n,  (numeric) How many zOHMC to generate.\n"
    ...
    CzOHMCWallet* zwallet = pwalletMain->zwalletMain;
...
UniValue dzohmcstate(const UniValue& params, bool fHelp) {
    if (fHelp || params.size() != 0)
        throw runtime_error(
                "dzohmcstate\n"
                "\nThe current state of the mintpool of the deterministic zOHMC wallet.\n" +
                HelpRequiringPassphrase() + "\n"
    ...
    obj.push_back(Pair("dzohmc_count", nCount));
...
void static SearchThread(CzOHMCWallet* zwallet, int nCountStart, int nCountEnd)
{
    ...
    try {
        ...
        for(int i = nCountStart; i < nCountEnd; i++) {
            ...
            CKey key;
            zwallet->SeedToZOHMC(zerocoinSeed, bnValue, bnSerial, bnRandomness, key);
...
UniValue searchdzohmc(const UniValue& params, bool fHelp)
{
    if(fHelp || params.size() != 3)
        throw runtime_error(
                "searchdzohmc\n"
                "\nMake an extended search for deterministically generated zOHMC that have not yet been recognized by the wallet.\n" +
                HelpRequiringPassphrase() + "\n"

                                             "\nArguments\n"
                                             "1. \"count\"       (numeric) Which sequential zOHMC to start with.\n"
                                             "2. \"range\"       (numeric) How many zOHMC to generate.\n"
                                             "3. \"threads\"     (numeric) How many threads should this operation consume.\n"

                                             "\nExamples\n" +
                HelpExampleCli("searchdzohmc", "1, 100, 2") + HelpExampleRpc("searchdzohmc", "1, 100, 2"));
    ...
    boost::thread_group* dzohmcThreads = new boost::thread_group();
    ...
    for (int i = 0; i < nThreads; i++) {
        ...
        nPrevThreadEnd = nEnd;
        dzohmcThreads->create_thread(boost::bind(&SearchThread, zwallet, nStart, nEnd));
    }

    dzohmcThreads->join_all();

    zwallet->RemoveMintsFromPool(pwalletMain->zohmcTracker->GetSerialHashes());
