...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 OHMC at the same time
    ONLY_10000 = 5,                        // find karmanode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zOHMC send
enum ZerocoinSpendStatus {
    ZOHMC_SPEND_OKAY = 0,                            // No error
    ZOHMC_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZOHMC_WALLET_LOCKED = 2,                         // Wallet was locked
    ZOHMC_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZOHMC_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZOHMC_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZOHMC_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZOHMC_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZOHMC_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZOHMC_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZOHMC_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZOHMC_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZOHMC_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZOHMC_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZOHMC_SPENT_USED_ZOHMC = 14,                      // Coin has already been spend
    ZOHMC_TX_TOO_LARGE = 15,                         // The transaction is larger than the max tx size
    ZOHMC_SPEND_V1_SEC_LEVEL
};
...
public:
    ...
    void ZOhmcBackupWallet();
    bool GetZerocoinKey(const CBigNum& bnSerial, CKey& key);
    bool CreateZOHMCOutput(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    bool GetMint(const uint256& hashSerial, CZerocoinMint& mint);
    bool GetMintFromStakeHash(const uint256& hashStake, CZerocoinMint& mint);
    bool DatabaseMint(CDeterministicMint& dMint);
    bool SetMintUnspent(const CBigNum& bnSerial);
    bool UpdateMint(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom);
    string GetUniqueWalletBackupName(bool fzohmcAuto) const;
    ...
    */
    ...
    CzOHMCWallet* zwalletMain;

    bool fFileBacked;
    bool fWalletUnlockAnonymizeOnly;
    std::string strWalletFile;
    bool fBackupMints;
    std::unique_ptr<CzOHMCTracker> zohmcTracker;
...
std::set<int64_t> setKeyPool;
    ...
    bool fMultiSendKarmanodeReward;
    ...
    void SetNull()
    {
        ...
        fMultiSendKarmanodeReward = false;
    ...
    void setZWallet(CzOHMCWallet* zwallet)
    {
        zwalletMain = zwallet;
        zohmcTracker = std::unique_ptr<CzOHMCTracker>(new CzOHMCTracker(strWalletFile));
    }

    CzOHMCWallet* getZWallet() { return zwalletMain; }
    ...
    void setZOhmcAutoBackups(bool fEnabled)
    {
    ...
    {
        return fMultiSendKarmanodeReward || fMultiSendStake;
    }
    ...
    /// Get 1000DASH output and keys which can be used for the Karmanode
    bool GetKarmanodeVinAndKeys(CTxIn& txinRet, CPubKey& pubKeyRet, CKey& keyRet, std::string strTxHash = "", std::string strOutputIndex = "");
