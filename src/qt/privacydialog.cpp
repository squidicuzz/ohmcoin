                                                          ...
                                                          currentBalance(-1)
{
    ...
    ui->labelzOHMCSyncStatus->setText("(" + tr("out of sync") + ")");
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zOHMCpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzOHMC_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zOHMC...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zOHMC in ") +
...
void PrivacyDialog::on_pushButtonSpendzOHMC_clicked()
{
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zOHMC
        sendzOHMC();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zOHMC
    sendzOHMC();
}

void PrivacyDialog::on_pushButtonZOhmcControl_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ZOhmcControlDialog* zOhmcControl = new ZOhmcControlDialog(this);
    zOhmcControl->setModel(walletModel);
    zOhmcControl->exec();
}

void PrivacyDialog::setZOhmcControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzOHMCSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzOHMC()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!IsValidDestinationString(ui->payTo->text().toStdString())) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid Ohmcoin Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zOHMCpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zOHMC is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " OHMC </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zOHMC</b>";
    ...
    // use mints from zOHMC selector if applicable
    vector<CMintMeta> vMintsToFetch;
    vector<CZerocoinMint> vMintsSelected;
    if (!ZOhmcControlDialog::setSelectedMints.empty()) {
        vMintsToFetch = ZOhmcControlDialog::GetSelectedMints();
        ...
        for (auto& meta : vMintsToFetch) {
            if (meta.nVersion < libzerocoin::PrivateCoin::PUBKEY_VERSION) {
                ...
                if (nSecurityLevel < 100) {
                    QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Version 1 zOHMC require a security level of 100 to successfully spend."), QMessageBox::Ok, QMessageBox::Ok);
                    ui->TEMintStatus->setPlainText(tr("Failed to spend zOHMC"));
                    ui->TEMintStatus->repaint();
                    return;
                }
            }
    ...
    if (!fSuccess) {
        if (receipt.GetStatus() == ZOHMC_SPEND_V1_SEC_LEVEL) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Version 1 zOHMC require a security level of 100 to successfully spend."), QMessageBox::Ok, QMessageBox::Ok);
            ui->TEMintStatus->setPlainText(tr("Failed to spend zOHMC"));
            ui->TEMintStatus->repaint();
            return;
    ...
    if (walletModel && walletModel->getAddressTableModel()) {
        // If zOHMC was spent successfully update the addressbook with the label
    ...
    // Clear zohmc selector in case it was used
    ZOhmcControlDialog::setSelectedMints.clear();
    ui->labelzOHMCSelected_int->setText(QString("0"));
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zOHMC Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " Ohmc, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zOHMC Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    std::vector<CMintMeta> vMints = pwalletMain->zohmcTracker->GetMints(false);
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zOHMC </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zOHMC "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zOHMC "));
    ui->labelzOHMCAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzOHMCSyncStatus->setVisible(fShow);
}
...
void PrivacyDialog::updateSPORK18Status()
{
    ...
    if (fMaintenanceMode && fButtonsEnabled) {
        ...
        ui->pushButtonMintzOHMC->setToolTip(tr("zOHMC is currently disabled due to maintenance."));

        // Spend zOHMC
        ui->pushButtonSpendzOHMC->setEnabled(false);
        ui->pushButtonSpendzOHMC->setToolTip(tr("zOHMC is currently disabled due to maintenance."));
    } else if (!fMaintenanceMode && !fButtonsEnabled) {
        // Mint zOHMC
        ui->pushButtonMintzOHMC->setEnabled(true);
        ui->pushButtonMintzOHMC->setToolTip(tr("PrivacyDialog", "Enter an amount of OHMC to convert to zOHMC", 0));

        // Spend zOHMC
        ui->pushButtonSpendzOHMC->setEnabled(true);
        ui->pushButtonSpendzOHMC->setToolTip(tr("Spend Zerocoin. Without 'Pay To:' address creates payments to yourself."));
    }
}