                                          ...
                                          banTableContextMenu(0)
{
    ...
    std::string strzohmcPathCustom = GetArg("-zohmcbackuppath", "");
    ...
    if(!strzohmcPathCustom.empty()) {
        ui->wallet_customzohmcbackuppath->setText(QString::fromStdString(strzohmcPathCustom));
        ui->wallet_customzohmcbackuppath_label->setVisible(true);
        ui->wallet_customzohmcbackuppath->setVisible(true);
    }

    if((!strPathCustom.empty() || !strzohmcPathCustom.empty()) && nCustomBackupThreshold > 0) {
...
void RPCConsole::setClientModel(ClientModel* model)
{
    ...
    if (model && clientModel->getPeerTableModel() && clientModel->getBanTableModel()) {
        ...
        setKarmanodeCount(model->getKarmanodeCountString());
        connect(model, SIGNAL(strKarmanodesChanged(QString)), this, SLOT(setKarmanodeCount(QString)));
...
void RPCConsole::setKarmanodeCount(const QString& strKarmanodes)
{
    ui->karmanodeCount->setText(strKarmanodes);
}
